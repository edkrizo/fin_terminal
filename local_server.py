...
async def generate_audio_endpoint(payload: AudioPayload):
    ...
    # 🚀 THE FIX: Parse the new God-Mode Dashboard Context
    context_str = ""
    if payload.dashboard_context:
        insights = payload.dashboard_context.get('insights', '')
        news = " ".join(payload.dashboard_context.get('top_news', []))
        movers = " ".join([f"{m.get('ticker')}: {m.get('chg')}" for m in payload.dashboard_context.get('market_movers', [])])
        catalysts = " ".join([f"{c.get('date')}: {c.get('event')}" for c in payload.dashboard_context.get('upcoming_catalysts', [])])
        
        context_str = f"Dashboard Insights: {insights}. \nTop News Headlines: {news}. \nActive Market Movers: {movers}. \nUpcoming Catalyst Calendar: {catalysts}"
    else:
        context_str = payload.text
...
if __name__ == "__main__":
    print("✅ Local Mock Server running on http://127.0.0.1:8080", flush=True)
    uvicorn.run("local_server:app", host="127.0.0.1", port=8080, reload=True)