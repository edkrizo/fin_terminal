...
from mercury_client.components.chat_interface import render_chat_interface
from mercury_client.components.financial_dashboard import render_main_workspace
from mercury_client.components.multimodal_video import render_news_dedicated_view

# 🚀 NEW: Importing the dynamic sidebar component we created in sidebar.py
from mercury_client.components.sidebar import left_sidebar

def top_nav() -> rx.Component:
    return rx.hstack(
        rx.hstack(
            rx.text("FACTSET", font_weight="900", font_size="18px", color="#FFF", letter_spacing="1px"),
            rx.badge("Ask Mercury", bg="#00A1E0", color="#FFF", border_radius="12px", padding="2px 8px"),
            spacing="3", align_items="center"
        ),
        rx.input(placeholder="Search FactSet...", bg="#1F2937", border="none", color="white", width="300px", height="30px", border_radius="15px", padding_left="15px"),
        rx.spacer(),
        rx.hstack(
            rx.text("Persona:", font_size="12px", color="#A1A1AA"),
            rx.select(AppState.personas, value=AppState.current_persona, on_change=AppState.set_persona, bg="#1F2937", color="#FFF", border="none", width="180px", height="30px"),
...
def index() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            # 1. Sidebar (15%) - Now dynamically rendered from our new modular component!
            left_sidebar(),
            ...
            rx.box(
                ...
                width="60%", padding="15px", bg="#F4F7FA", overflow_y="auto"
            ),
            
            # 3. Persistent Mercury Chat (25%)
            rx.box(
...
app.add_page(index, title="FactSet Workspace", on_load=AppState.initialize_dashboard)